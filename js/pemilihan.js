document.addEventListener("DOMContentLoaded", () => {
    ...
    const candidatesData = [
        { id: 1, name: "PASLON - 1", members: [{ name: "Vina", role: "Calon Ketua" }, { name: "Hadi", role: "Calon Wakil" }], image: "https://placehold.co/300x225/1e293b/ffffff?text=Vina+%26+Hadi", vision: "Menjadi wadah aspirasi dan kreativitas yang unggul, berintegritas, dan mampu beradaptasi dengan perubahan zaman.", mission: ["Menyelenggarakan forum diskusi rutin untuk menampung aspirasi.", "Membuat platform digital untuk berekspresi dan berkolaborasi.", "Menjalin kemitraan strategis dengan pihak eksternal.", "Meningkatkan transparansi dan akuntabilitas dalam pengelolaan organisasi."], witnesses: [{ name: "Gilang" }, { name: "Farah" }] },
        { id: 2, name: "PASLON - 2", members: [{ name: "Salsabila", role: "Calon Ketua" }, { name: "Bima", role: "Calon Wakil" }], image: "https://placehold.co/300x225/1e293b/ffffff?text=Salsabila+%26+Bima", vision: "Mewujudkan organisasi yang inovatif, kolaboratif, dan berdampak positif bagi seluruh anggota serta lingkungan sekitar.", mission: ["Meningkatkan kualitas program kerja yang berorientasi pada pengembangan skill.", "Membangun sinergi yang kuat antara anggota dan pengurus.", "Mengadakan kegiatan sosial yang bermanfaat bagi masyarakat.", "Menciptakan lingkungan organisasi yang terbuka dan suportif."], witnesses: [{ name: "Dewi" }, { name: "Eko" }] },
        { id: 3, name: "PASLON - 3", members: [{ name: "Nadia", role: "Calon Ketua" }, { name: "Rizki", role: "Calon Wakil" }], image: "https://placehold.co/300x225/1e293b/ffffff?text=Nadia+%26+Rizki", vision: "Terwujudnya generasi pemimpin yang berkarakter, cerdas, dan peduli terhadap sesama.", mission: ["Mengoptimalkan program kaderisasi kepemimpinan yang terstruktur.", "Mengadakan pelatihan soft skill dan hard skill secara berkala.", "Mendorong partisipasi aktif dalam kompetisi akademik dan non-akademik.", "Menginisiasi gerakan peduli lingkungan dan sosial."], witnesses: [{ name: "Indra" }, { name: "Joko" }] }
    ];
    ...
    const candidateGridContainer = document.getElementById("candidate-grid-container");
        ...
        return `
            <div class="candidate-card w-72 rounded-lg p-3 flex flex-col text-left">
                <div class="mb-1.5 px-1">
                    <h3 class="text-base tracking-wider page-heading">${candidate.name}</h3>
                    <p class="text-xs text-slate-400">${memberNames}</p>
                </div>
                <div class="relative w-full aspect-[4/3] mb-1.5">
                    <img src="${candidate.image}" alt="${memberNames}" class="w-full h-full object-cover rounded-lg shadow-md" onerror="this.onerror=null;this.src='https://placehold.co/300x225/334155/e2e8f0?text=Image+Not+Found'">
                </div>
                ...
                <div class="text-center mt-auto">
                    ...
                    <div class="flex justify-center items-center gap-4">
                        ${candidate.witnesses.map((witness) => `
                            <div class="text-center">
                                <img src="https://placehold.co/36x36/475569/e2e8f0?text=${witness.name.charAt(0)}" class="w-9 h-9 rounded-full mx-auto mb-1 border border-slate-600" alt="${witness.name}">
                                <p class="text-xs font-medium">${witness.name}</p>
                            </div>
                        `).join("")}
                    </div>
                    <button class="button-primary w-full py-1.5 mt-2 rounded-lg font-semibold text-sm" data-candidate-id="${candidate.id}">
                        DETAIL
                    </button>
                </div>
            </div>
    ...
    const populateCandidateGrid = () => {
        candidateGridContainer.innerHTML = candidatesData.map(generateCandidateCardHTML).join("");
    };

    const generateModalContentHTML = (candidate) => {
        const missionListItems = candidate.mission.map((item) =>
        ...
        modalDialog.innerHTML = `
            <div class="p-6 md:p-8">
                <div class="flex justify-between items-start mb-6">
                    <h2 class="text-3xl font-bold text-white page-heading">${candidate.name}</h2>
                ...
                <div class="flex flex-col md:flex-row gap-8">
                    <div class="md:w-1/3 flex-shrink-0">
                        <img src="${candidate.image}" alt="${candidate.name}" class="w-full object-cover rounded-xl shadow-lg">
                    ...
                    <div class="md:w-2/3 pr-2 max-h-[50vh] overflow-y-auto modal-scroll-content">
                        ...
                        <ul class="space-y-3 text-slate-300">${missionListItems}</ul>
                    </div>
                </div>
                <div class="mt-8 text-center">
                    <button id="modal-vote-button" class="primary-vote-button text-white font-bold py-3 px-12 rounded-full text-lg" ${hasVoted ? "disabled" : ""}>
                        ${hasVoted ? "SUARA TELAH DIBERIKAN" : `PILIH ${candidate.name}`}
                    </button>
                </div>
            </div>
        `;
        ...
        document.getElementById("modal-vote-button").addEventListener("click", () => processVote(candidate));
    };
    ...
    const disableVoting = (reason) => {
        ...
        document.querySelectorAll(".button-primary, .primary-vote-button").forEach((button) => {
    ...
    const processVote = (candidate) => {
        if (hasVoted) return;
        disableVoting(`Terima kasih! Suara Anda untuk ${candidate.name} telah direkam.`);
    };
